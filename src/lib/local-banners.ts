...
const slots = ['left', 'center', 'right_top', 'right_bottom', 'mobile'];
const directory = join(process.cwd(), '.local-banners');
const mobile: AdminHomepageBanner = {
  ...bannerCompositionDefaults, slot: 'mobile', title: null, subtitle: null,
...
export async function withLocalBanners<T extends HomepageBanner>(banners: T[]): Promise<T[]> {
  ...
  if (!merged.has('mobile')) merged.set('mobile', mobile);
